...
CVCUR<- function(x0.final,theta.final,beta.final,subid,data.path.u,obstime.u,N.u,h.u,knots.u,lambda1.final,lambda2.final,ini.fix=FALSE, maxloop=10,thre=0.0001,Phi.pen=NULL){
...
#############################################
  ...
  y.tilde <- xpath(N.u,h.u,x0.final,theta.final,beta.final,knots.u)        ### sample path evaluated at current estimates
  temp <- xderiv(N.u,h.u,theta.final,beta.final,knots.u,y.tilde)

  z.tilde <- temp[[1]]                                          ### gradient w.r.t. a 
  v.tilde <- temp[[2]]                                          ### gradient w.r.t. theta  
  w.tilde <- xderiv.beta(N.u,h.u,theta.final,beta.final,knots.u,y.tilde)       ### gradient w.r.t. beta

  temp2 <- xHess(N.u,h.u,theta.final,beta.final,knots.u,y.tilde,z.tilde,v.tilde)
  zz.tilde <- temp2[[1]]                                        ### Hessian w.r.t. a 
  vv.tilde <- temp2[[2]]                                        ### Hessian w.r.t. theta
  ww.tilde <- xHess.beta(N.u,h.u,theta.final,beta.final,knots.u,y.tilde,w.tilde)     ### Hessian w.r.t. beta
...
#### esimate x0.drop 
   ...
   if(ini.fix==FALSE){
     ...
     while((abs(x0.drop.old[i] - x0.drop.new[i])/sd(x0.final) > thre)  && (loopcount <= maxloop)){  
       ...
       y.drop <- xpath(N.u,h.u,x0.drop.old[i],theta.drop[i],beta.drop[,i],knots.u)     
       z.drop <- xderiv.a(N.u,h.u,theta.drop[i],beta.drop[,i],knots.u,y.drop)  ### gradient w.r.t. a                   
     ...
     y.drop <- xpath(N.u,h.u,x0.drop[i],theta.drop[i],beta.drop[,i],knots.u)       
